...
function plotTorquePredictions(figNum, name, t, tau_actual , tau_predict)
    e_tau = tau_actual - tau_predict;
    tau_rms = rms(e_tau);
    J_rms = sqrt( mean((vecnorm(e_tau')).^2 ) );
    ...
    rms_tag = sprintf('(%s) [J_RMS = %.2f, RMS=%.2f, %.2f, %.2f (Nm)]',name, J_rms, tau_rms(1), tau_rms(2), tau_rms(3));
